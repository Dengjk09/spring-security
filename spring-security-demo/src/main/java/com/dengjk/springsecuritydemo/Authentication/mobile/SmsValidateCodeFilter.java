...
public class SmsValidateCodeFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (StringUtils.equalsIgnoreCase("/authentication/mobile", request.getRequestURI()) && StringUtils.equalsIgnoreCase(request.getMethod(), "POST")) {
            /**开始验证验证码*/
            try {
    ...
    private void validateImageCode(HttpServletRequest request) throws ServletRequestBindingException, ValidateCodeException {
        ...
        if(codeInSession.isExpried()){
            sessionStrategy.removeAttribute(requestAttributes,  ValidateCodeProcessor.SESSION_KEY_PREFIX+"SMS");
